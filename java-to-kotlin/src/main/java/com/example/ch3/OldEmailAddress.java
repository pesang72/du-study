...
public class OldEmailAddress {
    private final String localPart; // <1>
    private final String domain;

    public static OldEmailAddress parse(String value) { // <2>
        var atIndex = value.lastIndexOf('@');
        if (atIndex < 1 || atIndex == value.length() - 1)
            throw new IllegalArgumentException(
                "EmailAddress must be two parts separated by @"
            );
        return new OldEmailAddress(
            value.substring(0, atIndex),
            value.substring(atIndex + 1)
        );
    }

    public OldEmailAddress(String localPart, String domain) { // <3>
    ...
    public boolean equals(Object o) { // <5>
        ...
        OldEmailAddress that = (OldEmailAddress) o;
